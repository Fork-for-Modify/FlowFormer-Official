...
class FlowDataset(data.Dataset):
    def __init__(self, aug_params=None, sparse=False,load_occlusion=False):
        self.augmentor = None
        self.sparse = sparse
    ...
    def __getitem__(self, index):
        ...
        if self.load_occlusion:
            ...
            occlusion = np.array(occlusion)[...,0].astype(np.float32)
...
class MpiSintel(FlowDataset):
    def __init__(self, aug_params=None, split='training', dstype='clean', image_root=None, flow_root=None, occlu_root=None):
        super(MpiSintel, self).__init__(aug_params=aug_params, load_occlusion=occlu_root is not None)

        if split == 'test':
            self.is_test = True

        for scene in sorted(os.listdir(osp.join(image_root))):
            if scene.split('-')[0]!=dstype:
                continue
            ...
            if split != 'test':
                ...
                if occlu_root is not None:
                    self.occ_list += sorted(glob(osp.join(occlu_root, scene, '*.png')))
...
def fetch_dataloader(args, TRAIN_DS='C+T+K+S+H'):
    ...
    elif args.stage == 'sintel_inr': # zzh
        aug_params = {'crop_size': args.image_size, 'min_scale': -0.2, 'max_scale': 0.6, 'do_flip': True}
        sintel_clean = MpiSintel(aug_params, split='training', dstype='clean', image_root=args.image_root, flow_root=args.flow_root)
        sintel_final = MpiSintel(aug_params, split='training', dstype='final', image_root=args.image_root, flow_root=args.flow_root)
